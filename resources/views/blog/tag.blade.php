...
@section('title', $tag->name . ' - Blog - Aj Group')
@section('meta_description', 'Explore articles, tutorials, and insights tagged with ' . $tag->name . ' from Aj Group.')
@section('og_title', $tag->name . ' - Blog - Aj Group')
@section('og_description', 'Explore articles, tutorials, and insights tagged with ' . $tag->name . ' from Aj Group.')
...
@section('content')
    ...
    <section class="hero">
        <div class="container text-center">
            ...
            <p>Explore articles and insights with this tag</p>
        </div>
    </section>
                            ...
                            <div class="card mb-4">
                                <div class="row g-0">
                                    <div class="col-md-4">
                                        ...
                                        @else
                                            <img src="https://via.placeholder.com/300x200?text=Aj+Group" class="img-fluid rounded-start h-100 w-100 object-fit-cover" alt="{{ $post->title }}">
                                        @endif
                                    </div>
                                    <div class="col-md-8">
                                        <div class="card-body">
                                            ...
                                            <div class="mb-2">
                                                ...
                                                @foreach($post->tags as $t)
                                                    <a href="{{ route('blog.tag', $t->slug) }}" class="badge bg-secondary text-decoration-none me-1">{{ $t->name }}</a>
                                                @endforeach
                                            </div>
                                ...
                                <ul class="list-group list-group-flush">
                                    @foreach($categories as $category)
                                        <li class="list-group-item d-flex justify-content-between align-items-center">
                                            <a href="{{ route('blog.category', $category->slug) }}" class="text-decoration-none">{{ $category->name }}</a>
                        ...
                        <div class="card-body">
                            ...
                            @if($tags->count() > 0)
                                <div class="d-flex flex-wrap gap-2">
                                    @foreach($tags as $t)
                                        <a href="{{ route('blog.tag', $t->slug) }}" class="badge bg-secondary text-decoration-none">{{ $t->name }}</a>
                                    @endforeach
                                </div>
                            @else
